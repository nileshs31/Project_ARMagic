...
public class DtwKnnRecognizer
{
    ...
    public float minMargin = 0.3f;          // min gap between best and 2nd-best class avg score; below this is ambiguous
    ...
    public (string label, float score, float margin) Recognize(List<Vector3> strokeWorld)
    {
        ...
        // reject if score too high (bad match) or margin too low (ambiguous)
        if (avgScore > acceptThreshold) return ("Unknown", avgScore, margin);
        if (margin < minMargin) return ("Unknown", avgScore, margin);
